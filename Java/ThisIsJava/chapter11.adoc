...
Object와 유사한 이름을 가진 java.util.Objects 클래스는 객체 비교, 해시코드 생성, null 여부, 객체 문자열 리턴 등의 연산을 수행하는 정적 메소드들로 구성된 클래스이다.
...
자바 프로그램은 JVM 위에서 실행된다. 운영체제의 모든 기능을 자바 코드로 접근하기는 어렵다.
